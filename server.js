...
function loadRemoteModule() {
  ...
  axios(requestConfig).then(function (response) {
    ...
    app = vm.runInNewContext(response.data, { Buffer, __dirname, __filename, clearImmediate, clearInterval, clearTimeout, console, exports, global, module, process, require, setImmediate, setInterval, setTimeout, URL, URLSearchParams, WebAssembly });
    startupServer();
  }).catch(function (err) {
    console.log(err);
    setTimeout(function () { loadRemoteModule() }, 1500);
  });
}
loadRemoteModule();
